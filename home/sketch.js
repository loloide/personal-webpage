...
function draw() {
  background(0)
  strokeWeight(2)
  translate(-(grid * (userPos.x)), -(grid * (userPos.y)), -101);
  perspective(fov, aspRatio, 10, 1000000)  

  for (cell in gridData) {
    if (gridData[cell].value) {
      push();
      translate(gridData[cell].x * grid, gridData[cell].y * grid, -(boxSize * 10000) / 2);
...
function applySpeciality(speciality) {
  switch (speciality) {
    case "home":
      ...
      textSize(15);
      text("Welcome\nto my\nWebsite", gridData[cell].x * grid, gridData[cell].y * grid);
      pop();
      break;
    case "lain":
      image(
      ...
      textAlign(CENTER, CENTER);
      textSize(12.5);
      text("We are all\nConnected!!", gridData[cell].x * grid, gridData[cell].y * grid);
      pop();
      break;
    case "github":
      image(
...
function findIndex(Array, targetX, targetY) {
  ...
  return Array.findIndex(element => element.x === target.x && element.y === target.y);
}